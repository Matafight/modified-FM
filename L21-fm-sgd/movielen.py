...
def sparsity_with_performance(data_name,x_train,y_train,x_test,y_test,num_attributes):
    ...
    for alpha in alpha_set:
        for mylambda in lambda_set:
            ...
            fm = pylibfm.FM(num_factors = num_factors,num_iter=1000,verbose = False,task="regression",initial_learning_rate=0.001,dataname=data_name,reg_1 = alpha, reg_2 = mylambda,gamma = 5)
            fm.fit(x_train,y_train,x_test,y_test,num_attributes)
            pre_label = fm.predict(x_test,y_test)
            diff = 0.5*np.sum((pre_label-y_test)**2)/y_test.size
            w_sparsity,total_sparsity = fm.return_sparsity()
            fh_sparsity_performance.write('w_sparsity:'+str(w_sparsity)+'\n')
            fh_sparsity_performance.write('total_sparsity:'+str(total_sparsity)+'\n')
