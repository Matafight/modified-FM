...
cdef class FM:
    ...
    cdef _sgd_theta_step_adadelta(self,DOUBLE *x_data_ptr,INTEGER * x_ind_ptr, int xnnz, DOUBLE y):
        ...
        for i in range(xnnz):
            ...
            self.delta_grad_w[feature] = -1*(np.sqrt(self.delta_squ_grad_w[feature]+1e-8)/np.sqrt(self.prev_grad_w[feature]+1e-8))*grad_w[feature]
            w[feature] += self.delta_grad_w[feature] 
            self.delta_squ_grad_w[feature] = 0.9*self.delta_squ_grad_w[feature] + 0.1*self.delta_grad_w[feature]**2
        ...
        for i in range(1,self.num_factors+1):
            for j in range(xnnz):
                ...
                self.delta_grad_v_p[feature,i] = -1*(np.sqrt(self.delta_squ_grad_v_p[feature,i]+1e-8)/np.sqrt(self.prev_grad_v_p[feature,i]+1e-8))*grad_v_p[feature,i]
                v_p[feature,i] += self.delta_grad_v_p[feature,i]
                self.delta_squ_grad_v_p[feature,i] = 0.9*self.delta_squ_grad_v_p[feature,i] + 0.1*self.delta_grad_v_p[feature,i]**2
