...
def performance_with_k(data_name,x_train,y_train,x_test,y_test,num_attributes,L_1,L_21):
    candidate_k = [20]
...
def sparsity_with_performance(data_name,x_train,y_train,x_test,y_test,num_attributes,L_1,L_21):
    ...
    for alpha in alpha_set:
        for mylambda in lambda_set:
            ...
            fm = pylibfm.FM(num_factors = num_factors,num_iter=1000,verbose = False,L_1 = L_1,L_21 = L_21task="regression",initial_learning_rate=0.001,dataname=data_name,reg_1 = alpha, reg_2 = mylambda,gamma = 5)
            fm.fit(x_train,y_train,x_test,y_test,num_attributes)
            pre_label = fm.predict(x_test,y_test)
            diff = 0.5*np.sum((pre_label-y_test)**2)/y_test.size
...
if __name__=='__main__':
    ...
    performance_with_k(train_data_name,x_train,train_label,x_test,test_label,num_attributes,L_1,L_21)
    #sparsity_with_performance(train_data_name,x_train,train_label,x_test,test_label,num_attributes,L_1,L_21)
